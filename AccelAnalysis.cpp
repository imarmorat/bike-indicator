...
void AccelAnalysis::init(Adafruit_ADXL345_Unified * adx345, uint32_t frequency, double multiplier, double min, double max, alarmCallbackFunc func)
{
	_adx345 = adx345;
...
void AccelAnalysis::update()
{
	...
	if (!isWithinLimit(_latest) && _func != NULL)
		_func(_latest);
}
